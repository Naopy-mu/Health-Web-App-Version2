...
/** 実装仕様書 5.3節の単位。 */
export const MEASUREMENT_UNITS = ["kg", "lb", "cm", "inch", "percent", "custom"] as const;
 ...
 */
export const MEASUREMENT_UNIT_CONSTRAINTS = ["mass", "percent", "length", "custom"] as const;
 ...
 * `percent -> percent`、`custom` は正規化しない（`null`）。
 *
...
export type NormalizedMeasurement = {
  ...
  readonly unit: Extract<MeasurementUnit, "kg" | "cm" | "percent">;
};
