...
>
> - 服用の記録・取消は**テーブルへ直接書けない**（原子的RPC経由のみ。1.4節）
...
### 1.4 在庫を動かす操作はサーバーの原子的RPCだけが行う（**重要**）
...
| `SUPPLEMENT_CONFLICT`           | 409  | 版番号不一致、または対象行が無い（更新・削除・取消）                                                 | 1.7節の手順で復帰する                       |
| `SUPPLEMENT_DUPLICATE_CONFLICT` | 409  | 商品名・商品キー・予定・ロット名の重複                                                               | 1.7節の後退手段で既存を探して編集へ倒す     |
...
`idempotencyKey` もこの2つから決定的に作ること（1.5節）。
...
- 対象が存在しない／自分のものでない／版番号が本当に食い違う場合は
  409 `SUPPLEMENT_CONFLICT`。1.7節の手順で復帰する。
...
ただし `lotCode` が無いと 1.7節の後退手段が使えないので、**登録直後に応答の `id` を
必ず保持すること**。
...
| `id`        | —        | 指定するとその1件だけ。**他の絞り込みと併用不可**（1.7節）                 |
...
| フィールド             | 意味                                                               |
| ---------------------- | ------------------------------------------------------------------ |
| `weeklyScheduledCount` | 直近7日間に発生するはずだった予定の回数（`as_needed` は数えない）  |
| `weeklyTakenCount`     | 直近7日間の服用回数（`taken` / `as_needed`。取消・スキップは除く） |
| `monthlyTakenCount`    | 直近30日間の服用回数                                               |
| `lowStockProductCount` | 低在庫の商品数（しきい値が設定され、残量合計がそれ以下）           |
| `expiringLotCount`     | 期限接近ロット数（残量があり、期限が30日以内。期限切れも含む）     |

`weeklyScheduledCount` は予定の展開規則（daily は毎日1回、weekly は対象曜日、
once は開始日、`as_needed` は 0回）に従ってサーバーが数える。日付の境界は
`Asia/Tokyo` のローカル日で切る。
...
`resource=movement` で読める追記専用の記録。**ブラウザからは書けない**（1.4節）。
