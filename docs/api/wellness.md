...
`idempotent_replay` の成功応答を返す**（1.5節の契約が優先する）。保存に成功した
あとで利用者がその種別をアーカイブし、送信キューに残っていた同じキーが後から
届く——という順序はオフライン同期（実装仕様書 8章）では普通に起こる。

先行リクエストの保存と種別のアーカイブが、再送の処理中（冪等キーを引いたあと、
書き込みの直前）に確定した場合も同じで、応答は `idempotent_replay` になる。
...
>
> **同時に投げても上限は破れない。** 件数の検査は所有者単位のトランザクション
> ロック（`pg_advisory_xact_lock`）で直列化してあるので、有効29件の状態から
> 2件を同時に作成／解除しても、通るのは片方だけで、もう片方は 400
> `WELLNESS_TYPE_LIMIT_REACHED` になる。直列化されるのは同じ所有者への同時変更
> だけで、他の利用者の操作は待たされない。
...
- アーカイブ済み種別への**新規登録**は 400 `WELLNESS_TYPE_ARCHIVED`。
  既存記録の更新と、適用済み `clientMutationId` の再送は妨げない（5.4節・6.2節）。
