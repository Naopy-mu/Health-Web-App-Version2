...
## Phase 4-2a（サプリメント・バックエンド）Nice-to-have バックログ

Codex 再レビューの Should-fix 3 件（更新0件の誤った重複競合・在庫ロット存在中の
`defaultUnit` 変更・S-2 の 409/404 契約不一致）は
`fix: correct conflict error codes and prevent unit mismatch on product update` で対応済み。
以下は同レビューの Nice-to-have で、**記録のみ**とした。

| #      | 項目                                                         | 内容                                                                                                                                                                                                                                                                                      | 記録理由・今回見送る判断                                                                                                                                                                                                                                 | 該当箇所                                                                                  |
| ------ | ------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- |
| P42a-1 | コメント内の「1.7節」への参照が API 契約の節番号とずれている | `docs/api/supplements.md` の「409 のあとに対象行を特定する」は 1.8節だが、`schema.ts`（2 箇所）と `conflict.ts`（2 箇所）の JSDoc は旧番号の「1.7節」を指したままになっている。現在の 1.7節は「保存は『全置換』」で、内容が一致しない。                                                   | 参照先の**内容**は正しく書かれており、読み手が迷うのは節番号だけ。節番号は今後の加筆でまた動くため、参照の書き方自体（節番号ではなく見出し文言で指す）を見直すのとあわせて直す方が、同じ修正を繰り返さずに済む。                                         | `src/features/supplements/schema.ts:344,362`、`src/features/supplements/conflict.ts:3,25` |
| P42a-2 | 検証状況表のテスト件数・migration 本数の更新漏れ             | `docs/api/supplements.md` 11節のテスト件数と冒頭表の migration 本数が、テスト追加・migration 追加のたびに手で直されておらず実態とずれていた（例: `supplements.test.ts` は 33 件と書かれていたが実際は 36 件、「ほか3件」と書かれていたが実際は 5 件）。**本コミットで現在値へ更新済み**。 | 数値そのものは更新したが、**手で書き写している限り同じずれが再発する**。恒久対策は件数・本数を文書から落とすか、CI で `vitest --reporter=json` の結果と `supabase/migrations` の本数を突き合わせて検証すること。次にこの表へ触るときにまとめて判断する。 | `docs/api/supplements.md` 冒頭表・11節                                                    |

### 単位変更とロット作成の同時実行（Codex 再レビュー Should-fix、対応済み）

`fix: serialize product unit change against concurrent lot creation` で対応。
商品の `defaultUnit` 変更と、同じ商品への在庫ロット作成が同時に走ると、両側の
トリガーが互いの未コミットの変更を見られず、商品 = g / ロット = tablet が残りえた。
migration `20260921000300` が両側のトリガーで商品単位の `pg_advisory_xact_lock` を
取って直列化する（キー空間は所有者単位の在庫ロックとは別。理由は migration 冒頭）。

**実 PostgreSQL での検証（実施済み）**: `tests/db/supplements-unit-race.pg.test.ts` が
Docker 上の PostgreSQL 16.15 / 17.11 で独立した2セッションを競合させる。修正前は
「単位変更が先」で不一致が1件残り（`mismatches: 1`）、修正後は4件とも成功。CI でも
`postgres:17-alpine` を立てて毎回実行する。PGlite 側（`supplements.test.ts`）は
ロックが実際に取られていることだけを確かめる（真の並行実行は再現できない）。

検証の過程で分かったこと（記録のみ）:

- **修正前でも「ロット作成が先」の順序は偶然守られていた。** `supplement_products` は
  一意キーに生成列 `name_normalized` を含み、BEFORE UPDATE トリガーがあると生成列は
  常に再計算扱いになる。そのため商品の UPDATE は FOR UPDATE 相当の行ロックを取り、
  ロット作成の外部キー検査（KEY SHARE）と衝突して待たされていた。これはロック強度の
  副作用であって設計ではないため、advisory lock で明示的に直列化した。
- **素朴に advisory lock を足すだけではデッドロックする**（40P01、実 PostgreSQL で
  再現）。単位変更は「商品行 → advisory」、ロット作成は「advisory → 外部キー検査の
  商品行」の順になるため。ロット側で先に商品行へ KEY SHARE を取り、獲得順を揃えた。
- 保証は **READ COMMITTED 前提**（ロック獲得後の検査が新しいスナップショットで読むこと
  に依存する）。PostgREST・RPC は既定の READ COMMITTED で動くので現状は問題ない。
  将来 REPEATABLE READ 以上で書き込む経路を足す場合は、直列化失敗（40001）の再試行を
  含めて見直すこと。
