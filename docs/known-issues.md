...
| P2-5 | 認証フローのE2E                  | 保護ルートのリダイレクト・`next` の丸め・API共通境界は Vitest で検証済み。実ブラウザでのログイン往復は未検証                                   | Playwright実行環境と Supabase ローカル環境       | 実装仕様書 12章          |
| P2-6 | `env:check` の実装               | Phase 0 の雛形のまま。認証で必須になった `NEXT_PUBLIC_SUPABASE_URL` / `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` の欠落検出は未実装                | 実装仕様書 13.1節の全変数の要否確定              | 実装仕様書 12章・13.1節  |
