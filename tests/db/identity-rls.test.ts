...
describe("users / user_profiles の RLS 分離 (実装仕様書 6.5節 / 9章)", () => {
  ...
  it("anon には共通関数の EXECUTE が付与されていない", async () => {
    ...
    expect(rows).toStrictEqual([
      ...
      // サプリメント（実装仕様書 5.6節）。在庫の整合性に関わる原子的RPCは
      // authenticated から呼べる必要があるため EXECUTE を与える（関数の中で
      // 所有者と利用者状態を検査する。migration 20260915000400）。
      { routine_name: "record_supplement_intake", anon: false, authenticated: true },
      ...
      // 定義域・正規化・集計は CHECK 制約と画面から使うため authenticated へ。
      // ロックのキー空間（supplement_inventory_lock_class）は RPC の内部都合なので与えない。
      { routine_name: "supplement_category_is_allowed", anon: false, authenticated: true },
      { routine_name: "supplement_form_is_allowed", anon: false, authenticated: true },
      { routine_name: "supplement_intake_status_is_allowed", anon: false, authenticated: true },
      { routine_name: "supplement_inventory_lock_class", anon: false, authenticated: false },
      { routine_name: "supplement_meal_relation_is_allowed", anon: false, authenticated: true },
      { routine_name: "supplement_movement_kind_is_allowed", anon: false, authenticated: true },
      { routine_name: "supplement_normalized_name", anon: false, authenticated: true },
      { routine_name: "supplement_product_stock", anon: false, authenticated: true },
      { routine_name: "supplement_schedule_kind_is_allowed", anon: false, authenticated: true },
      { routine_name: "supplement_summary", anon: false, authenticated: true },
      { routine_name: "supplement_unit_is_allowed", anon: false, authenticated: true },
      { routine_name: "supplement_weekdays_are_valid", anon: false, authenticated: true },
      ...
      { routine_name: "tg_supplement_lot_before_delete", anon: false, authenticated: false },
      { routine_name: "tg_supplement_lot_guard", anon: false, authenticated: false },
      { routine_name: "tg_supplement_product_unit_guard", anon: false, authenticated: false },
      { routine_name: "tg_supplement_record_movement", anon: false, authenticated: false },
      { routine_name: "tg_supplement_record_mutation", anon: false, authenticated: false },
      { routine_name: "tg_supplement_schedule_guard", anon: false, authenticated: false },
      ...
      { routine_name: "void_supplement_intake", anon: false, authenticated: true },
